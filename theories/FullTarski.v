...
Section Tarski.
  ...
  Definition exploding D (I : interp D) := forall rho phi, rho ⊨ (⊥ --> phi).
  Definition standard D (I : interp D ) := classical I /\ ((@i_F _ D I) -> False).
  Definition exp D (I : interp D) := classical I /\ exploding I.
  ...
  Lemma list_or_spec A D (I : interp D) rho :
    standard I -> (rho ⊨ list_or A <-> exists phi, phi el A /\ rho ⊨ phi).
  Proof.
    intros [_ Hstand]. induction A; cbn; split; auto.
    - firstorder.
    - firstorder.
    - intros [H|H].
      + exists a. tauto.
  ...
  Section Substs.
    ...
    Lemma sat_subst rho rho' sigma phi :
